...
const { convertZigbeeLocalTimeToLocalDateTime } = require('./util');
...
class HeimgardDoorLockCluster extends DoorLockCluster {
    ...
    static get ATTRIBUTES() {
        return {
            ...
            numberOfBiometricUsersSupported: {
                id: 27, type: ZCLDataTypes.uint16
            },
            enableLogging: {
    ...
    async onOperatingEventNotification(payload) {
        if (payload && payload !== null) {
            payload = {
                ...
                localTime: convertZigbeeLocalTimeToLocalDateTime(payload.localTime),
                data: payload.data,
            };
    ...
    async onProgrammingEventNotification(payload) {
        if (payload && payload !== null) {
            payload = {
                programmingEventSource: payload.programmingEventSource,
                programmingEventCode: payload.programmingEventCode,
                userID: payload.userID,
                pinCode: payload.pinCode,
            ...
            this.emit('event.programmingEventNotification', payload);
        }
    }
}
